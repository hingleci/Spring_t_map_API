									...
									$
											.ajax({
												...
												success : function(response) {
													...
													$("#result").text("");
													$("#result").append(
															tDistance + "<br/>"+ tTime + "<br/>"+  tFare + "<br/>"
																	+ taxiFare);
													...
													if (trafficInfochk == "Y") {
														for ( var i in resultData) { //for문 [S]
															...
															if (geometry.type == "LineString") {
																...
																/*트래픽값찾기*/
				                                                console.log("trafficArr.length :" + trafficArr.length);
				                                                console.log("trafficArr :" + trafficArr);
				                                                for (var u = 0; u < trafficArr.length; u++) {
				                           
				                                                   console.log("trafficArr["+u+"][2]: " + trafficArr[u][2]);
				                                                }
				                                                /* 영역*/
...
$(function() {	//밑에동작만으로는 안되서 $(function() 넣어주니 작동.........
	$("#btn_reset").on("click", function() {
		var dd = confirm("모든 마크, 경로 , 결과값이 사라집니다.");
		if(dd){
		//if(confirm("모든 마크, 경로 , 결과값이 사라집니다.")){
		console.log(dd);
			resettingMap();
			$("#result").text("");
			$("#marker_s").val("");
		...
		for(i=1;i<=cNo;i++){
			(function(m) {						
				...
				$("#"+m+"bt").on("click", function() {
					...
					$("#sbSearch").attr("action", "sbSearchProcess");
...
$(function() {
		...
		for(i=1;i<=cNo;i++){
			(function(m) {						
				...
				var dd = confirm("정말로 삭제하시겠습니가?");
				if(dd){
				
					//nno = $("#"+m+"del").val();
					nno = $(this).attr("data-no");
					
					$("#no").val(nno);
					
					
					$("#sbSearch").attr("action", "saveDelete");
					$("#sbSearch").attr("method", "post");
					$("#sbSearch").submit();
				} else {
		...
		for(i=1;i<=cNo;i++){
			(function(m) {						
				...
				$("#"+m+"up").on("click", function() {
					...
					noUP = $("#"+m+"up").val();
					$("#noUp").val(noUp);
					if($("#saveUpForm").css("display") == "block") {
							
						$("#saveUpForm").slideDown(300);
